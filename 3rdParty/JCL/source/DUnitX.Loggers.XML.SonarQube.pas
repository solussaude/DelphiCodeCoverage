...
type
  ...
  protected
    ...
    procedure OnTestingStarts(const threadId: TThreadID;
      testCount, testActiveCount: Cardinal); override;
    ...
    procedure FindDelphiUnits(const ASearchPath: String;
  ...
  public
    constructor Create(const ASourceDirsFilename: String;
      const AOutputStream: TStream; const AOwnsStream: boolean = false);
...
begin
  case Word(wc) of
    $0009, $000A, $000C, $000D, $0020 .. $D7FF, $E000 .. $FFFD,
    // Standard Unicode chars below $FFFF
...
function EscapeForXML(const value: string; const isAttribute: boolean = True;
  const isCDATASection: boolean = false): string;
begin
...
constructor TDUnitXXMLSonarQubeFileLogger.Create(const ASourceDirsFilename
  : String; const AFilename: string);
var
  sXmlFilename: string;
...
class function TDUnitXXMLSonarQubeFileLogger.
  GetSourcesDirFromCommandLine: String;
const
...
begin
  ...
  try
    try
      ...
      on E: Exception do
        System.WriteLn
          ('Error loading sources file, results cannot be imported in SonarQube. ('
          + E.Message + ')');
    end;

    for LSourceDir in LSourcesList do
    begin
...
{$ELSE}
  ...
  {$IFDEF USE_NS}System.{$ENDIF}SysUtils.ThousandSeparator;
...
function TDUnitXXMLSonarQubeLogger.FilenameByUnitName(const AUnitName
  : String): String;
begin
...
begin
  if FindFirst(ASearchPath + '\*', faAnyFile, searchResult) = 0 then
  begin
    try
      repeat
...
begin
  result := {$IFDEF USE_NS}System.{$ENDIF}SysUtils.Format(Format, Args,
    FFormatSettings);
end;
...
procedure TDUnitXXMLSonarQubeLogger.OnTestingEnds(const RunResults
  : IRunResults);
var
...
procedure TDUnitXXMLSonarQubeLogger.WriteFixtureResult(const fixtureResult
  : IFixtureResult);
var
...
procedure TDUnitXXMLSonarQubeLogger.WriteTestResult(const testResult
  : ITestResult);
begin
  WriteXMLLine('<testCase name="' + EscapeForXML(testResult.Test.Name) +
    '" duration="' + testResult.Duration.Milliseconds.ToString + '">');

  Indent;
  case testResult.ResultType of
    TTestResultType.MemoryLeak, TTestResultType.Error:
      WriteXMLLine('<error message="' + EscapeForXML(testResult.Message) + '">'
        + EscapeForXML(testResult.StackTrace, false) + '</error>');
    TTestResultType.Failure:
      WriteXMLLine('<failure message="' + EscapeForXML(testResult.Message) +
        '">' + EscapeForXML(testResult.StackTrace, false) + '</failure>');
    TTestResultType.Ignored:
      WriteXMLLine('<skipped message="' +
        EscapeForXML(testResult.Message) + '"/>');
  end;
  Outdent;
